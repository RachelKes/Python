...
class Video:
    ...
    def __check_name(self, name):
        ...
        if char_count < MIN_NAME_LENGTH:
            raise ValueError("Name must have at least " + str(MIN_NAME_LENGTH) + " characters that are letters or digits")

    def __check_length(self, length):
        if type(length) is not int:
            raise TypeError("Video constructor expects an integer value for length")
        if length < MIN_LENGTH or length > MAX_LENGTH:
            raise ValueError('Length must be at least ' + str(MIN_LENGTH) + " and no more than " + str(MAX_LENGTH))
    ...
    def __str__(self):
        return str(self.__collection_no) + ': ' + self.__name + ', ' + str(self.__length) + ' minutes, ' + self.__format
